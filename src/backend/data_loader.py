...
class DataLoader:
    def __init__(self, uploaded_files=None, db_config=None, sqlite_file=None):
        ...
        self.current_user = "adithprajwal"
        self.current_datetime = datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # Updated timestamp
    ...
    def load_sample_data(self):
        ...
        try:
            ...
            self.inventory_df = pd.read_csv('data/inventory_levels.csv') 
    ...
    def add_audit_columns(self):
        ...
        for df in [self.warehouses_df, self.sales_df, self.products_df, self.transport_df]:
            if df is not None:
    ...
    def get_urgent_orders(self, days_threshold: int = 2) -> pd.DataFrame:
        ...
        An order is urgent if its status is 'Urgent' OR its delivery deadline is near.
        """
        current_date = pd.to_datetime(self.current_datetime)
        # Condition 1: The order is explicitly marked as 'Urgent'
        status_is_urgent = self.sales_df['status'] == 'Urgent'
        # Condition 2: The delivery deadline is within the threshold
        deadline_is_near = (self.sales_df['delivery_deadline'] - current_date).dt.days <= days_threshold
        # Combine both conditions with an OR operator (|)
        return self.sales_df[status_is_urgent | deadline_is_near]
    ...
    def get_order_history(self, current_date: str, days_back: int = 7) -> pd.DataFrame:
        ...
        # Add status and time since order
        history['order_progress'] = np.where(history['date'] < current_date, 'Delivered', 'In Progress')
    ...
    def calculate_reorder_needs(self) -> pd.DataFrame:
        """Calculate which products need reordering based on detailed inventory levels."""
        reorder_needs = []
        # Group the inventory data by product_id and sum the stock
        product_stock = self.inventory_df.groupby('product_id')['current_stock'].sum()
        for _, product in self.products_df.iterrows():
            # Get the total stock for the current product
            total_stock = product_stock.get(product['product_id'], 0)
            # Compare with the reorder point
